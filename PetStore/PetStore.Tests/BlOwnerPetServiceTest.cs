...
{
    public class BlOwnerPetServiceTest
    {
        private readonly Mock<IPetRepository> _petRepositoryMock;
        private readonly List<Pet> _pets;

        public BlOwnerPetServiceTest()
        {
        ...
        [Fact]
        public void GetPetByOwnerId_ReturnsPetsForOwner()
        {
            // Arrange
            var ownerId = "1";
            var expectedPets = _pets.Where(p => p.OwnerId == ownerId).ToList();
            _petRepositoryMock.Setup(repo => repo.GetAll()).Returns(_pets);
            var blOwnerPetService = new BlOwnerPetService(_petRepositoryMock.Object);

            // Act
            var result = blOwnerPetService.GetPetByOwnerId(ownerId);

            // Assert
            Assert.NotNull(result);
            Assert.Equal(expectedPets.Count, result.Count);
            Assert.All(result, pet => Assert.Equal(ownerId, pet.OwnerId));
        }
    }
}
